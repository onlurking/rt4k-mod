...
describe("Parity tests - SNES.rt4 known values", () => {
  ...
  test("serializeValues contains all expected keys", () => {
    const parsed = JSON.parse(profile.serializeValues()) as Record<
      string,
      unknown
    >;

    const transmitter = (parsed["output"] as Record<string, unknown>)[
      "transmitter"
    ...
    expect(transmitter["hdr"]).toBeDefined();
    expect(transmitter["deep_color"]).toBeDefined();
  ...
  test("VRR Off round-trip - other parity values unchanged", async () => {
    ...
    expect(reloaded.getValue("input").asString()).toBeDefined();
    expect(reloaded.getValue("output.resolution").asString()).toBeDefined();
    expect(
      reloaded.getValue("output.transmitter.hdr").asString(),
    ).toBeDefined();
    expect(
      reloaded.getValue("output.transmitter.deep_color").asString(),
    ).toBeDefined();
  });

  test("multi-range write round-trip - input set to Front|Composite", async () => {
    const p = await RetroTinkProfile.build(SNES_FIXTURE);
    p.setValue("input", "Front|Composite");
  ...
  test("CRC validity - saved file has non-zero CRC bytes at [32] and [33]", async () => {
    ...
    const bytes = new Uint8Array(
      await Bun.file("/tmp/parity-crc-test.rt4").arrayBuffer(),
    );
