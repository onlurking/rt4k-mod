...
const tempFiles = [
  "/tmp/rt4k-profile-save-test.rt4",
  "/tmp/rt4k-profile-write-test.rt4",
];
...
describe("RetroTinkProfile", () => {
  describe("build", () => {
    it("resolves for a valid profile file", async () => {
      await expect(RetroTinkProfile.build(fixturePath)).resolves.toBeInstanceOf(
        RetroTinkProfile,
      );
    });

    it("throws InvalidProfileFormatError for invalid header bytes", () => {
      expect(() => RetroTinkProfile.fromBytes(new Uint8Array(23004))).toThrow(
        InvalidProfileFormatError,
      );
      expect(() => RetroTinkProfile.fromBytes(new Uint8Array(23004))).toThrow(
        "Header is invalid",
    ...
    it("throws ProfileNotFoundError for a nonexistent profile path", async () => {
      await expect(
        RetroTinkProfile.build("/tmp/rt4k-profile-does-not-exist.rt4"),
  ...
  describe("getValue", () => {
    it("reads output.transmitter.vrr as FreeSync from SNES.rt4", async () => {
      ...
      expect(profile.getValue("output.transmitter.vrr").asString()).toBe(
        "FreeSync",
      );
    });
  ...
  describe("serializeValues", () => {
    it("returns valid JSON containing output.transmitter.vrr", async () => {
      const profile = await RetroTinkProfile.build(fixturePath);
      const parsed = JSON.parse(profile.serializeValues()) as {
        output: { transmitter: { vrr: string } };
      };
  ...
  describe("save", () => {
    it("writes a profile that reloads and stores a non-zero CRC", async () => {
      ...
      const reloaded = await RetroTinkProfile.build(
        "/tmp/rt4k-profile-save-test.rt4",
      );
      const bytes = new Uint8Array(
        await Bun.file("/tmp/rt4k-profile-save-test.rt4").arrayBuffer(),
      );
      expect(reloaded.getValue("output.transmitter.vrr").asString()).toBe(
        "FreeSync",
      );
    ...
    it("preserves other values after setValue, save, and reload", async () => {
      ...
      expect(reloaded.getValue("output.resolution").asString()).toBe(
        before.resolution,
      );
      expect(reloaded.getValue("output.transmitter.hdr").asString()).toBe(
        before.hdr,
      );
    });
  ...
  describe("getSettingsNames", () => {
    it("returns writable setting names only", async () => {
      ...
      for (const readOnlySetting of SCHEMA.filter(
