...
function intDef(
  type: DataType.INT | DataType.SIGNED_INT | DataType.BIT,
): SettingDef {
  return {
    name: `test.${type}`,
    desc: "Test integer",
...
describe("RetroTinkSettingValue", () => {
  describe("compareUint8Array", () => {
    it("compares byte arrays byte-by-byte", () => {
      expect(
        RetroTinkSettingValue.compareUint8Array(
          new Uint8Array([1]),
          new Uint8Array([1]),
        ),
      ).toBe(true);
  ...
  describe("ENUM", () => {
    it("decodes bytes to enum names", () => {
      const settingValue = new RetroTinkSettingValue(
        vrrDef,
        new Uint8Array([0x01]),
      );
    ...
    it("supports multi-byte enum comparison", () => {
      const input = new RetroTinkSettingValue(inputDef, new Uint8Array([5, 0]));
  ...
  describe("STR", () => {
    ...
    it("writes strings as char codes with null termination", () => {
      ...
      expect(settingValue.value).toEqual(
        new Uint8Array([83, 78, 69, 83, 0, 0, 0, 0]),
      );
    });
    ...
    it("filters null bytes while decoding strings", () => {
      const settingValue = new RetroTinkSettingValue(
        strDef(),
        new Uint8Array([83, 0, 78, 69, 83, 0]),
      );
  ...
  describe("asPlainObject", () => {
    it("builds nested objects with enum string values", () => {
      ...
      expect(settingValue.asPlainObject()).toEqual({
        output: { transmitter: { vrr: "FreeSync" } },
      });
    });
    ...
    it("builds nested objects with bit boolean values", () => {
      ...
      expect(settingValue.asPlainObject()).toEqual({
        output: { transmitter: { deep_color: true } },
      });
    });
  });
    ...
    it("uses SettingValidationError message format", () => {
      try {
        ...
        expect((err as Error).message).toContain(
          "(output.transmitter.vrr) failed validation with (",
        );
