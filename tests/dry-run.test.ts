...
describe("--dry-run mode", () => {
  test("single file dry-run prints changes without creating files", () => {
    ...
    const { stdout, exitCode } = run(["modify", "--file", path.join(FIXTURES, "SNES.rt4"), "--output-dir", DRY_OUT, "--vrr", "Off", "--dry-run"]);
  ...
  test("dry-run exits with code 0", () => {
    const { exitCode } = run(["modify", "--file", path.join(FIXTURES, "SNES.rt4"), "--output-dir", DRY_OUT, "--vrr", "Off", "--dry-run"]);
    expect(exitCode).toBe(0);
  });

  test("directory dry-run shows all files that would be modified", () => {
    const { stdout, exitCode } = run(["modify", "--input-dir", FIXTURES, "--output-dir", DRY_OUT, "--vrr", "Off", "--dry-run"]);
    expect(exitCode).toBe(0);
  ...
  test("dry-run does NOT write any files to output-dir", () => {
    fs.rmSync(DRY_OUT, { recursive: true, force: true });
    run(["modify", "--input-dir", FIXTURES, "--output-dir", DRY_OUT, "--vrr", "Off", "--dry-run"]);
    expect(fs.existsSync(DRY_OUT)).toBe(false);
  });
